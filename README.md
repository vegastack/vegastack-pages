...
- **Manual bearer.** Sign in, open **Settings → My Connections**, click **Create token**, copy the value. Useful for headless agents, CI, or MCP clients that need a static bearer. Pass via `vpg login --token <token>` or `VPG_TOKEN`.
