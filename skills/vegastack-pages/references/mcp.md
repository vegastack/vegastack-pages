...
- `get_page`: read metadata with optional `include: ["source", "rendered", "versions", "comments", "publication"]`.
...
- `update_thread`: reply, resolve, reopen, or complete a thread. Pass `agent_name`, `agent_model`, and `agent_session_id` for agent-attributed replies.
...
- `publication_apply` creates or updates page/folder publications. Pass `publication_id` when updating by id.
...
After creating a page, publish it with comment permission via `publication_apply` when needed, return the page URL to the user, then call `wait_for_review` with `timeout_ms: 600000`. If comments arrive, call `list_comments`, inspect `anchor_context`, patch with concurrency tokens, then `update_thread` for each handled thread. Use text anchors for Markdown/MDX and point anchors for HTML pins. If the host agent is interrupted by a new user message, stop waiting and answer that message first.
