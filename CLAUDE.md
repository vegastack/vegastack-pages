# Claude Code rules for vegastack-pages

You are working inside the vegastack-pages monorepo. This file is loaded into
your context on every conversation. Internalize the release gate **before**
doing anything that could mutate remote state.

## Release gate (hard)

Do **not**, in any conversation, do any of the following without the
maintainer's explicit, named release approval **in this conversation**:

- `git push` (any branch, any remote)
- `git tag` followed by `git push --tags` or `git push <remote> <tag>`
...
After Claude shows the relevant draft and impact summary, short approval gates
...
- Run `pnpm install`, `pnpm typecheck`, `pnpm test`, `pnpm build`,
  `pnpm format`.
- Run `pnpm changeset` (which only writes a local file in `.changeset/`).
- Run `pnpm run version-sync` locally to preview a version bump (it edits
  local files — does not push or publish).
...
Full procedure lives in `.agents/skills/ship/SKILL.md`. When the maintainer
asks to ship, cut a release, publish, or deploy, follow that skill. CLAUDE
should treat the skill as the authoritative source of truth for the release
flow; if a step in CLAUDE.md and the skill ever disagree, the skill wins —
...
.agents/skills/ship/  This release workflow
```
