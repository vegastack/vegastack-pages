    ...
    const timeout = window.setTimeout(async () => {
      try {
        const response = await fetch(
          `/api/search?workspace_id=${encodeURIComponent(workspaceId)}&q=${encodeURIComponent(query)}&limit=8`,
        ...
        if (!query.trim()) {
          const favoritesResponse = await fetch(
            `/api/workspaces/${encodeURIComponent(workspaceId)}/favorites`,
            { signal: controller.signal },
          );
          if (favoritesResponse.ok) {
            const favoritePayload = (await favoritesResponse.json()) as {
              favorites: Array<{
                page_id: string;
                title: string;
                url: string;
            ...
            setFavorites(
              favoritePayload.favorites.slice(0, 6).map((favorite) => ({
                type: "page",
                id: favorite.page_id,
