...
- **Agent session**: a workspace-scoped bearer token issued via OAuth 2.1 + PKCE (browser MCP clients), manual creation from **Settings → My Connections**, or `vpg login --token`. Every member can create their own (`kind=manual`); workspace admins can revoke any session in the workspace from **Settings → Connections Log**. Tokens are shown once and stored hashed.
